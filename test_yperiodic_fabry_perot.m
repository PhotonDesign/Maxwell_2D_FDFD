...
Source = zeros(M,N);
Source(:,round(N*0.7)) = 1/h; %a line source
...
T = zeros(1,n);
for i= 1:n
    Matz(:,round(N*0.3):round(N*(Layer_Thickness(i)+0.3))) = const;
    Sol = Scattering_Solve(omega,Dim,h,BC,Matx,Maty,Matz, Source);
    difference = Sol(round(M/2),:)-Vac_Sol(round(M/2),:);
    T(i) = abs(Sol(round(M/2), round(T_plane*N))/Vac_Sol(round(M/2),round(T_plane*N)))^2;
    R(i) = abs(difference(round(R_plane*N))/Vac_Sol(round(M/2),round(R_plane*N)))^2;
end

%analytical solution
r = -(1-sqrt(const))/(1+sqrt(const));
phi = omega*sqrt(const)*(Layer_Thickness*Dim(1)+h);
